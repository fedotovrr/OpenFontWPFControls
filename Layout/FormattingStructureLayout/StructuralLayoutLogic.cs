    ...
    public static class StructuralLayoutLogic
    {
        public static List<StructuralContainer> GetContainers(this IContainersCollection structure, StructuralLayout layout, out float width, out float height)
        {
            ...
            static List<StructuralContainer> CreateContainers(IContainersCollection structure, StructuralLayout textLayout, float maxWidth, float x, float y, out float width, out float height)
            {
                ...
                List<(TextAlignment align, StructuralContainer container)> alignments = new ();
                double localWidth = 0;
                ...
                if (structure != null && maxWidth > 0)
                {
                    foreach (object o in structure.Items)
                    {
                        switch (o)
                        {
                            case IInlineCollection inlines:
                                ...
                                result.Add(structuralContainer);

                                List<StructuralLine> lines = structuralContainer.GetLines(maxWidth);
                                structuralContainer.Lines.AddRange(lines);
                                foreach (StructuralLine line in lines)
                                {
                                    ...
                                    localWidth = Math.Max(line.Width, width);
                                    height += lineHeight;
                                    ...
                                    //        {
                                    //            structuralContainer.Hyperlinks.Add(new HitBox(lastContainer.Text, lastContainer.X, y - lineHeight, current.X - lastContainer.X, lineHeight));
                                    //        }
                                    ...
                                    //if (lastContainer.Text?.HitObject != null)
                                    //{
                                    //    structuralContainer.Hyperlinks.Add(new HitBox(lastContainer.Text, lastContainer.X, y - lineHeight, lastPoint.X + lastPoint.Width - lastContainer.X, lineHeight));
                                    //}
                                }

                                alignments.Add(inlines is ITextAlignment alignment ? (alignment.TextAlignment, structuralContainer) : (TextAlignment.Left, structuralContainer));
                            ...
                            case ITable table:
                                ...
                                // add content cells and calc cell size
                                ...
                                if (tableBorder != null)
                                {
                                    ...
                                    localWidth = Math.Max(tableBorder.Width, width);
                                ...
                                else
                                {
                                    ...
                                    localWidth = Math.Max(columnsWidths.Sum(), width);
                    ...
                    foreach ((TextAlignment align, StructuralContainer container) in alignments)
                    {
                        // align text
                        switch (align)
                        {
                            case TextAlignment.Right:
                                foreach (StructuralLine line in container.Lines)
                                {
                                    line.XOffset = (float)(localWidth - line.Width);
                                }
                                break;
                            case TextAlignment.Center:
                                foreach (StructuralLine line in container.Lines)
                                {
                                    line.XOffset = (float)((localWidth - line.Width) / 2);
                                }
                                break;
                        }
                        ...
                        foreach (StructuralLine line in container.Lines)
                        {
                            ...
                            foreach (GlyphInfo current in line.GlyphPoints)
                            {
                                if (current.Text != lastContainer.Text)
                                {
                                    if (lastContainer.Text?.HitObject != null)
                                    {
                                        container.Hyperlinks.Add(new HitBox(lastContainer.Text, line.XOffset + lastContainer.X, line.YOffset, current.X - lastContainer.X, lineHeight));
                                    }
                                    lastContainer = current;
                                }
                                lastPoint = current;
                            ...
                            if (lastContainer.Text?.HitObject != null)
                            {
                                container.Hyperlinks.Add(new HitBox(lastContainer.Text, line.XOffset + lastContainer.X, line.YOffset, lastPoint.X + lastPoint.Width - lastContainer.X, lineHeight));
                            }
                        }
                    }
                }
