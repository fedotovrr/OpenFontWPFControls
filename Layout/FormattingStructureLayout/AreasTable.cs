...
namespace OpenFontWPFControls.Layout.FormattingStructureLayout
{
    internal class AreasTable
    {
        ...
        public AreasTable(IEnumerable<IPlacement> items)
        {
            ...
            foreach (IPlacement item in items)
            {
                int colIndex = Clamp(item.XOffset / CellWidth, 0, _cols - 1);
                int rowIndex = Clamp(item.YOffset / CellHeight, 0, _rows - 1);
                _areas[rowIndex, colIndex].Add(item);
            }
        }
        ...
        public IEnumerable<T> GetItems<T>(Rect bounds) where T : IPlacement
        {
            if (_areas != null && _areas.GetLength(0) > 0 && _areas.GetLength(1) > 0)
            {
                ...
                float height = double.IsInfinity(bounds.Width) ? float.MaxValue : (float)bounds.Width;
