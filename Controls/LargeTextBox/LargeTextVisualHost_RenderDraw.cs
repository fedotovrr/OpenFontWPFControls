...
namespace OpenFontWPFControls.Controls
{
    internal partial class LargeTextVisualHost
    {
        ...
        private void UpdateContentLayer()
        {
            ...
            foreach ((LargeTextLine line, double y) in LineInViewBounds())
            {
        ...
        public void UpdateControlLayer()
        {
            ...
            if (_layout?.CaretPoints.Any() == true)
            {
                ...
                DrawCaret(context, _parentControl?.GetValue(LargeTextBox.CaretBrushProperty) as Brush ?? Brushes.Black);
                UpdateCaretVisible();
            }
                ...
                if (line.CaretPointContains(_caretPoint.CharOffset))
                {
                    foreach (CaretPoint current in line.CaretPoints)
                    {
                        if (current.Equals(_caretPoint))
                        {
                            ...
                            caret.Margin = new Thickness(Math.Min(current.X, _viewWidth - caret.Width), y, 0, 0);
                            _caretVisible = true;
