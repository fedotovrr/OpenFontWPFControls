...
namespace OpenFontWPFControls.Controls
{
    ...
    {
        private IEnumerable<(LargeTextLine line, double y)> LineInViewBounds()
        {
            if (_drawingBounds.Height > 0)
            {
                foreach ((LargeTextLine line, double y) tuple in ViewLine())
                {
        ...
        private IEnumerable<(LargeTextLine line, double y)> ViewLine()
        {
            if (_layout != null)
            {
                double height = 0;
        ...
        {
            LargeTextLine first = null;
            LargeTextLine last = null;
            int count = 0;
            double height = 0;
            foreach ((LargeTextLine line, double _) in ViewLine())
            {
                first ??= line;
                last = line;
                count++;
            ...
            if (first != null && last != null && count > 0 && _layout?.GetLine(_caretPoint) is LargeTextLine curLine)
            {
